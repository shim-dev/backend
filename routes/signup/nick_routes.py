...
@nick_bp.route('/get_nickname', methods=['POST'])
def get_nickname():
    data = request.json
    user_id = data.get('user_id')
    if not user_id:
        return jsonify({'success': False, 'message': 'user_id 누락됨'}), 400

    try:
        object_id = ObjectId(user_id)
    except Exception:
        return jsonify({'success': False, 'message': 'user_id 형식이 잘못됨'}), 400

    user = users.find_one({'_id': object_id})
    if user:
        return jsonify({'success': True, 'nickname': user.get('nickname', '사용자')})
    else:
        return jsonify({'success': False, 'message': '사용자를 찾을 수 없음'}), 404
